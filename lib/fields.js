...
export class LineField {
    ...
    debug(ctx) {
        // Requires a 2D canvas context to render the line
        // This is optional and only needed if you want to visualize the field
        if (!ctx) return;
        const x1 = 0;
        const y1 = this.pos[1] + this.slope * (x1 - this.pos[0]);
        const x2 = ctx.canvas?.width || 800;
        const y2 = this.pos[1] + this.slope * (x2 - this.pos[0]);
        ctx.strokeStyle = 'red';
        ctx.beginPath();
        ctx.moveTo(x1, y1);
        ctx.lineTo(x2, y2);
